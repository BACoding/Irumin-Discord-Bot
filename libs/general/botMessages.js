...
module.exports = {
  helpMain(bot, helpList, version) {
    return embedMessage(
      'IRUMIN | HOW TO USE',
      'Try typing `!help [with command]` for more info.',
      helpList,
      '',
      `IRUMIN ${version}`,
      bot.avatarURL()
  ...
  helpCommand(user, command, prefix) {
    return embedMessage(
      `IRUMIN | HOW TO USE [ ${prefix + command.name.toUpperCase()} ]`,
      command.description,
      '',
      command.image,
      '',
      command.aliases.length >= 1 ? `• You can also use [ ${prefix + command.aliases} ]` : ''
    );
  },
  invalidCommand() {
    return embedMessage(
      'INVALID COMMAND',
      "That command doesn't exist. Check !help for a list of the available commands."
    );
  },
  botPermissions() {
  ...
  messagesDeleted(message, msgCleaned) {
    ...
    if (msgCleaned === 1) qtyString = 'message';
