...
const { IRUMIN_EMOJIS } = requireOrFallback(
  `./config/config.json`,
...
function parseEmoji (str) {
  ...
  return {
    ...
    name: groups.name,
    id: groups.id,
    identifier: `${groups.animated ?? ``}${groups.name ?? ``}${
      groups.id ? `:${groups.id}` : ``
    }`,
  };
}
...
function getEmojiOrFallback (message, key = `🤖`) {
  if (!(message instanceof Message) || !(key in IRUMIN_EMOJIS)) return key;
  const { name, id } = parseEmoji(IRUMIN_EMOJIS[key]);
  const foundEmojis = message.client.emojis.cache.filter(
    (e) => e.id === id || e.name === name
  );
  const emojiInCurrentGuild = foundEmojis.find(
    (e) => e.guild.id === message.guild.id
  );
