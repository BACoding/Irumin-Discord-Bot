...
module.exports = {
  ...
  execute (message) {
    ...
    channel.messages
      .fetch({ limit: 100 })
      .then((messages) =>
        messages.filter((msg) => msg.deletable && msg.author.id === me.id)
      )
      .then((ownMsgs) => {
      ...
      .then((deleted) => {
        ...
        channel
          .send(botMsg.messagesDeleted(message.member.id, numDeleted, bulk))
          .then((msg) => {
            if (msg) return msg.delete({ timeout: 5000 });
          });
      });
  },
};
