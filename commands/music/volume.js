  ...
  execute (message, args) {
    ...
    if (!queue)
      return message.channel
        .send(
          embedMessage(
            `Up! Down! It's all in the mind 🔊`,
            `But there's nothing going on so... ${
              IRUMIN_TUCC ? IRUMIN_TUCC : ``
            }`
          )
        )
        .catch(console.error);
    ...
    if (!args[0])
      return message.channel
        .send(
          embedMessage(
            `Up! Down! It's all in the mind 🔊`,
            `Volume is currently at **${queue.volume}%** power`
          )
        )
        .catch(console.error);
    ...
    if (parseInt(args[0]) > 100 || parseInt(args[0]) < 0)
      return message.channel
        .send(
          embedMessage(
            `Up! Down! It's all in the mind 🔊`,
            `The volume only goes from 0 to 100!`
          )
        )
        .catch(console.error);
