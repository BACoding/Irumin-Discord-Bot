  ...
  async execute(message) {
    ...
    if (!queue)
      return message.channel
        .send(embedMessage("Let's stop for a second", `Eh? There's nothing playing ${IRUMIN_TUCC ? IRUMIN_TUCC : ''}`))
        .catch(console.error);
    ...
    if (queue.playing) {
      ...
      return message.channel
        .send(embedMessage("Let's stop for a second", `Stay tunned after this quick break sponsored by ${message.author}`))
        .catch(console.error);
    }
  }
};
