...
module.exports = {
  ...
  async execute(message) {
    ...
    if (!queue.playing) {
      ...
      return message.channel.send(embedMessage(`We're back!`,`With more just for you, thanks to our sponsor ${message.author} for the quick break`))
        .catch(console.error);
    }

    return message.channel.send(embedMessage(`We're back!`,`Or we would be if there was something playing ${IRUMIN_SMUG ? IRUMIN_SMUG : ''}`))
        .catch(console.error);
  }
};