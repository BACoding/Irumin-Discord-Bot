  ...
  async execute(message) {
    ...
    if (!serverQueue)
      return message.channel.send(
        embedMessage('So what do we have today...', `Eeeeh? There's nothing playing ${IRUMIN_TUCC ? IRUMIN_TUCC : ''}`)
      );
    ...
    for (const [index, song] of serverQueue.songs.entries()) {
      ...
      if (songNumber === 1) {
        queueString.push(
          `**${songNumber}. [${song.title}](${song.url}) [[${convertMs(serverQueue.connection.dispatcher.streamTime)}](${
            song.url
          }&t=${Math.round(serverQueue.connection.dispatcher.streamTime / 1000)}) / ${convertSeconds(song.duration)}]**`
        );
      } else if (songNumber === QUEUE_LIMIT) {
        queueString.push(
          `**Stay tunned there's more! This is HateGrooves FM! Can't stop won't stop** ${IRUMIN_TATAKAI ? IRUMIN_TATAKAI : ''}`
        );
        break;
      } else {
