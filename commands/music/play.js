  ...
  async execute (message, args) {
    ...
    if (!channel) {
      channel = message.guild.channels.cache.find(
        (channel) => channel.id === botConfig.MUSICROOM_ID
      );
      if (!channel)
        return message.channel
          .send(
            `User is not in a voice channel and bot's music room does not exist.`
          )
          .catch(console.error);
    }
    ...
    if (botConfig.CHECK_USER_VC) {
      if (!channel)
        return message.channel
          .send(botMsg.userActivity(message.member.id))
          .catch(console.error);
    ...
    if (!args.length)
      return message.channel
        .send(botMsg.emptyCommand(message.member.id))
        .catch(console.error);
    ...
    if (urlValid) {
      try {
        ...
        return message.channel
          .send(botMsg.invalidUrl(message.member.id))
          .catch(console.error);
      }
    } else {
      try {
    ...
    if (serverQueue) {
      ...
      return serverQueue.textChannel
        .send(botMsg.songAdded(song.requester, song.title))
        .catch(console.error);
    }
    ...
    try {
      ...
      await channel.leave();
      return message.channel
        .send(botMsg.botError(message.member.id))
        .catch(console.error);
    }
  },
};
